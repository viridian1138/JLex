/**************************************************************
  ...
  Version 1.2.2-G.1, 8/20/98, [Thorn Green]
  Notes:
    Changed the code generated by the "emit" routines.  Instead of emitting
     a single ".java" code file, two files are emitted: a ".java" code file
     like that in version 1.2.2, and a ".dat" file which contains the
     translation tables.  The compiled version of the ".java" output loads
     the ".dat" file as a resource, and then reads the translation tables
     from it.  This change is intended to solve problems caused by JDK 1.2,
     and the Java VM that ships with Netscape Navigator 4.x for Windows.  In
     particular, these products will not execute a Java method longer than 64K.
     This affects translation tables declared as array literals.  The javac
     compiler translates an int[] literal to a piece of code that individually
     initializes each element of the array.  As a result, the length of the
     bytecode generated is roughly proportional to the number of elements in
     the array.  If the lexer's declared arrays become large enough, the 
     "initialization" method of the lexer class will become larger than 64K, 
     and certain VMs will refuse to execute it.  Placing the data in an object
     ".dat" file, that loads at runtime, overcomes this problem.
...
class CEmit {
	...
	private ObjectOutputStream o_outstream;
	...
	/***************************************************************
	  ...
	  **************************************************************/
	private void set(CSpec spec, OutputStream outstream, ObjectOutputStream outstream2) {
		if (CUtility.DEBUG) {
			CUtility.jlex_assert(null != spec);
			CUtility.jlex_assert(null != outstream);
			CUtility.jlex_assert(null != outstream2);
		}

		m_spec = spec;
		m_outstream = new DataOutputStream(outstream);
		o_outstream = outstream2;
	}
	  ...
	  (
	   ...
	   OutputStream outstream
	    ...
	    throws java.io.IOException      
		{
		  ...
		  if (CUtility.DEBUG)
		    {
		      CUtility.jlex_assert(null != m_spec);
		      CUtility.jlex_assert(null != m_outstream);
		    }*/
	...
	/***************************************************************
	  ...
	  **************************************************************/
	void emit(CSpec spec, OutputStream outstream, ObjectOutputStream outstream2) throws java.io.IOException {
		set(spec, outstream, outstream2);

		if (CUtility.DEBUG) {
			CUtility.jlex_assert(null != m_spec);
			CUtility.jlex_assert(null != m_outstream);
			CUtility.jlex_assert(null != o_outstream);
		}
	...
	private void emit_construct() throws java.io.IOException {
		...
		m_outstream.writeBytes("\tprivate final char YYEOF = \'\\uFFFF\';\n");

		/* User specified class code. */
		if (null != m_spec.m_class_code) {
			m_outstream.writeBytes(new String(m_spec.m_class_code, 0, m_spec.m_class_read));
		}
		...
		if (null != m_spec.m_init_throw_code) {
			...
			m_outstream.writeBytes(new String(m_spec.m_init_throw_code, 0, m_spec.m_init_throw_read));
			m_outstream.writeBytes("\n\t\t{\n");
		}
		else {
			m_outstream.writeBytes(" {\n");
		...
		m_outstream.writeBytes(
			"\t\tjava.io.ObjectInputStream MyStr = yy_getBaseStream( \""
				+ (new String(m_spec.m_class_name))
				+ ".dat\" , true );\n");
		m_outstream.writeBytes("\t\ttry { \n");
		m_outstream.writeBytes("\t\t\tyy_acpt = (int[])( MyStr.readObject() );\n");
		m_outstream.writeBytes("\t\t\tyy_cmap = (int[])( MyStr.readObject() );\n");
		m_outstream.writeBytes("\t\t\tyy_rmap = (int[])( MyStr.readObject() );\n");
		m_outstream.writeBytes("\t\t\tyy_nxt = (int[][])( MyStr.readObject() );\n");
		m_outstream.writeBytes("\t\t\tMyStr.close();\n");
		m_outstream.writeBytes("\t\t} catch( Exception e ) { e.printStackTrace(); }\n");
		...
		if (null != m_spec.m_init_code) {
			m_outstream.writeBytes(new String(m_spec.m_init_code, 0, m_spec.m_init_read));
		}
	...
	private void emit_states() throws java.io.IOException {
		...
		while (states.hasMoreElements()) {
			...
			if (CUtility.DEBUG) {
				CUtility.jlex_assert(null != state);
			}
	...
	private void emit_helpers() throws java.io.IOException {
		...
		if (null != m_spec.m_eof_code) {
			...
			if (null != m_spec.m_eof_throw_code) {
				m_outstream.writeBytes("\n");
				m_outstream.writeBytes("\t\tthrows ");
				m_outstream.writeBytes(new String(m_spec.m_eof_throw_code, 0, m_spec.m_eof_throw_read));
				m_outstream.writeBytes("\n\t\t{\n");
			}
			else {
				m_outstream.writeBytes(" {\n");
			...
			m_outstream.writeBytes(new String(m_spec.m_eof_code, 0, m_spec.m_eof_read));
			m_outstream.writeBytes("\t\t}\n");
		...
		/* Function: yy_getBaseStream */
		m_outstream.writeBytes("\tpublic java.io.ObjectInputStream yy_getBaseStream( String in , boolean res )\n");
		m_outstream.writeBytes("\t\t{\n");
		m_outstream.writeBytes("\t\tjava.io.InputStream u = null;\n");
		m_outstream.writeBytes("\t\tjava.io.ObjectInputStream v = null;\n");
		m_outstream.writeBytes("\t\t\n");
		m_outstream.writeBytes("\t\tif( res ) { try{ u =\n");
		m_outstream.writeBytes(
			"\t\t\tnew java.io.BufferedInputStream( "
				+ (new String(m_spec.m_class_name, 0, m_spec.m_class_name.length))
				+ ".class.getResourceAsStream( in ) );\n");
		m_outstream.writeBytes("\t\t\tv = new java.io.ObjectInputStream( u );\n");
		m_outstream.writeBytes("\t\t\t} catch( Exception e ) { /* System.out.println( e ); */ } }\n");
		m_outstream.writeBytes("\n");
		m_outstream.writeBytes("\t\tif( v != null )\n");
		m_outstream.writeBytes("\t\t\t{ /* System.out.println( \"Wow!!!!! Got Resource!!!!!!!!!!\" ); */ }\n");
		m_outstream.writeBytes("\t\t\telse System.out.println( \"Assert: Did not get Resource!!!!!!!!!!\" );\n");
		m_outstream.writeBytes("\n");
		m_outstream.writeBytes("\t\treturn( v );\n");
		m_outstream.writeBytes("\t\t}\n\n");
	...
	private void emit_header() throws java.io.IOException {
		...
		m_outstream.writeBytes(new String(m_spec.m_class_name, 0, m_spec.m_class_name.length));
		if (m_spec.m_implements_name.length > 0) {
			m_outstream.writeBytes(" implements ");
			m_outstream.writeBytes(new String(m_spec.m_implements_name, 0, m_spec.m_implements_name.length));
		}
		m_outstream.writeBytes(" {\n");
	}
	...
	private void emit_table() throws java.io.IOException {
		...
		m_outstream.writeBytes("\tprivate int yy_acpt[] = null;\n\n");
		size = m_spec.m_accept_vector.size();
		int[] tmp_yy_acpt = new int[size];
		for (elem = 0; elem < size; ++elem) {
			...
			if (null != accept) {
				...
				if (is_start && true == is_end) {
					tmp_yy_acpt[elem] = YY_START | YY_END;
				}
				else if (is_start) {
					tmp_yy_acpt[elem] = YY_START;
				}
				else if (is_end) {
					tmp_yy_acpt[elem] = YY_END;
				}
				else {
					tmp_yy_acpt[elem] = YY_NO_ANCHOR;
				}
			}
			else {
				tmp_yy_acpt[elem] = YY_NOT_ACCEPT;
			}

		}

		o_outstream.writeObject(tmp_yy_acpt);
		tmp_yy_acpt = null;

		m_outstream.writeBytes("\tprivate int yy_cmap[] = null;\n\n");
		int[] tmp_yy_cmap = new int[m_spec.m_col_map.length];
		for (i = 0; i < m_spec.m_col_map.length; ++i)
			tmp_yy_cmap[i] = m_spec.m_col_map[i];

		o_outstream.writeObject(tmp_yy_cmap);
		tmp_yy_cmap = null;

		m_outstream.writeBytes("\tprivate int yy_rmap[] = null;\n\n");
		int[] tmp_yy_rmap = new int[m_spec.m_row_map.length];
		for (i = 0; i < m_spec.m_row_map.length; ++i)
			tmp_yy_rmap[i] = m_spec.m_row_map[i];

		o_outstream.writeObject(tmp_yy_rmap);
		tmp_yy_rmap = null;

		m_outstream.writeBytes("\tprivate int yy_nxt[][] = null;\n\n");
		size = m_spec.m_dtrans_vector.size();
		int[][] tmp_yy_nxt = new int[size][m_spec.m_dtrans_ncols];
		for (elem = 0; elem < size; ++elem) {
			dtrans = (CDTrans) m_spec.m_dtrans_vector.elementAt(elem);

			for (i = 0; i < m_spec.m_dtrans_ncols; ++i)
				tmp_yy_nxt[elem][i] = dtrans.m_dtrans[i];

		}

		o_outstream.writeObject(tmp_yy_nxt);
		tmp_yy_nxt = null;
	...
	private void emit_driver() throws java.io.IOException {
		...
		if (null != m_spec.m_yylex_throw_code) {
			m_outstream.writeBytes(", ");
			m_outstream.writeBytes(new String(m_spec.m_yylex_throw_code, 0, m_spec.m_yylex_throw_read));
			m_outstream.writeBytes("\n\t\t{\n");
		}
		else {
			m_outstream.writeBytes(" {\n");
		...
		if (NOT_EDBG) {
			m_outstream.writeBytes("java.lang.System.out.println(\"Current state: \"" + " + yy_state\n");
			m_outstream.writeBytes("+ \"\tCurrent input: \"\n");
			m_outstream.writeBytes(" + ((char) yy_lookahead));\n");
		}
		if (NOT_EDBG) {
			m_outstream.writeBytes("\t\t\tjava.lang.System.out.println(\"State = \"" + "+ yy_state);\n");
			m_outstream.writeBytes(
			...
			m_outstream.writeBytes(
				"\t\t\tjava.lang.System.out.println(\"Lookahead input = \"" + "+ ((char) yy_lookahead));\n");
		}
		...
		m_outstream.writeBytes("\t\t\t\tif (YYEOF == yy_lookahead " + "&& true == yy_initial) {\n");
		if (null != m_spec.m_eof_code) {
		...
		if (m_spec.m_integer_type || true == m_spec.m_yyeof) {
			...
			m_outstream.writeBytes(new String(m_spec.m_eof_value_code, 0, m_spec.m_eof_value_read));
		}
		else {
	...
	private void emit_actions(String tabs) throws java.io.IOException {
		...
		if (CUtility.DEBUG) {
			CUtility.jlex_assert(m_spec.m_accept_vector.size() == m_spec.m_anchor_array.length);
		}
		...
		for (elem = 0; elem < size; ++elem) {
			...
			if (null != accept) {
				...
				m_outstream.writeBytes(new String(accept.m_action, 0, accept.m_action_read));
				m_outstream.writeBytes("\n");
...
class CMakeNfa {
	...
	private void set(CLexGen lexGen, CSpec spec, CInput input) {
		if (CUtility.DEBUG) {
			CUtility.jlex_assert(null != input);
			CUtility.jlex_assert(null != lexGen);
			CUtility.jlex_assert(null != spec);
		}
	...
	private void expr(CNfaPair pair) throws java.io.IOException {
		...
		if (CUtility.DEBUG) {
			CUtility.jlex_assert(null != pair);
		}
	...
	private void factor(CNfaPair pair) throws java.io.IOException {
		...
		if (CLexGen.CLOSURE == m_spec.m_current_token
			...
			if (CLexGen.CLOSURE == m_spec.m_current_token || CLexGen.OPTIONAL == m_spec.m_current_token) {
				start.m_next2 = end;
			}

			if (CLexGen.CLOSURE == m_spec.m_current_token || CLexGen.PLUS_CLOSE == m_spec.m_current_token) {
	...
	private void term(CNfaPair pair) throws java.io.IOException {
		...
		if (CLexGen.OPEN_PAREN == m_spec.m_current_token) {
			...
			if (false == (CLexGen.ANY == m_spec.m_current_token || CLexGen.CCL_START == m_spec.m_current_token)) {
				...
				if (CLexGen.ANY == m_spec.m_current_token) {
	...
	private void dodash(CSet set) throws java.io.IOException {
		...
		while (CLexGen.EOS != m_spec.m_current_token && CLexGen.CCL_END != m_spec.m_current_token) {
			if (CLexGen.DASH == m_spec.m_current_token) {
				if (-1 == first) {
					CError.parse_error(CError.E_DASH, m_input.m_line_number);
				}
				if (CUtility.DEBUG) {
					CUtility.jlex_assert(-1 != first);
				}
...
class CMinimize {
	...
	private void reduce() {
		...
		for (reduced_ncols = 0;; ++reduced_ncols) {
			if (CUtility.DEBUG) {
				for (i = 0; i < reduced_ncols; ++i) {
					CUtility.jlex_assert(-1 != m_spec.m_col_map[i]);
				}
			}
			...
			if (CUtility.DEBUG) {
				CUtility.jlex_assert(false == set.get(i));
				CUtility.jlex_assert(-1 == m_spec.m_col_map[i]);
			}
		...
		if (CUtility.DEBUG) {
			CUtility.jlex_assert(k == reduced_ncols);
		}
		...
		for (reduced_nrows = 0;; ++reduced_nrows) {
			if (CUtility.DEBUG) {
				for (i = 0; i < reduced_nrows; ++i) {
					CUtility.jlex_assert(-1 != m_spec.m_row_map[i]);
				}
			}
		...
		if (CUtility.DEBUG) {
			...
			CUtility.jlex_assert(k == reduced_nrows);
		}
	}
	...
	private void minimize() {
		...
		while (old_group_count != group_count) {
			...
			if (CUtility.DEBUG) {
				CUtility.jlex_assert(m_group.size() == group_count);
			}
				...
				for (j = 1; j < group_size; ++j) {
					...
					for (c = 0; c < m_spec.m_dtrans_ncols; ++c) {
						...
						if (goto_first != goto_next
							...
							if (CUtility.DEBUG) {
								CUtility.jlex_assert(dtrans_group.elementAt(j) == next);
							}
							...
							if (CUtility.DEBUG) {
								CUtility.jlex_assert(m_group.contains(new_group) == true);
								CUtility.jlex_assert(m_group.contains(dtrans_group) == true);
								CUtility.jlex_assert(dtrans_group.contains(first) == true);
								CUtility.jlex_assert(dtrans_group.contains(next) == false);
								CUtility.jlex_assert(new_group.contains(first) == false);
								CUtility.jlex_assert(new_group.contains(next) == true);
								CUtility.jlex_assert(dtrans_group.size() == group_size);
								CUtility.jlex_assert(i == m_ingroup[first.m_label]);
								CUtility.jlex_assert((m_group.size() - 1) == m_ingroup[next.m_label]);
							}
	...
	private void init_groups() {
		...
		for (i = 0; i < size; ++i) {
			...
			if (CUtility.DEBUG) {
				CUtility.jlex_assert(i == dtrans.m_label);
				CUtility.jlex_assert(false == group_found);
				CUtility.jlex_assert(group_count == m_group.size());
			}
			...
			for (j = 0; j < group_count; ++j) {
				...
				if (CUtility.DEBUG) {
					CUtility.jlex_assert(false == group_found);
					CUtility.jlex_assert(0 < dtrans_group.size());
				}
				...
				if (CUtility.SLOW_DEBUG) {
					...
					s = dtrans_group.size();
					CUtility.jlex_assert(0 < s);

					for (k = 1; k < s; ++k) {
						check = (CDTrans) dtrans_group.elementAt(k);
						CUtility.jlex_assert(check.m_accept == first.m_accept);
					}
				}
				...
				if (first.m_accept == dtrans.m_accept) {
					...
					if (CUtility.DEBUG) {
						CUtility.jlex_assert(j == m_ingroup[dtrans.m_label]);
					}
...
class CNfa2Dfa {
	...
	/* throws java.lang.CloneNotSupportedException*/ {
		...
		for (istate = 0; nstates > istate; ++istate) {
			...
			while (null != (dfa = get_unmarked())) {
				...
				if (CUtility.DEBUG) {
					CUtility.jlex_assert(false == dfa.m_mark);
				}
				...
				for (i = 0; i < m_spec.m_dtrans_ncols; ++i) {
					if (CUtility.DEBUG) {
						CUtility.jlex_assert(0 <= i);
						CUtility.jlex_assert(m_spec.m_dtrans_ncols > i);
					}
					...
					if (CUtility.DEBUG) {
						CUtility.jlex_assert(nextstate < m_spec.m_dfa_states.size());
					}
				...
				if (CUtility.DEBUG) {
					CUtility.jlex_assert(m_spec.m_dtrans_vector.size() == dfa.m_label);
				}
	...
	private void e_closure(CBunch bunch) {
		...
		/* Debug checks. */
		if (CUtility.DEBUG) {
			CUtility.jlex_assert(null != bunch);
			CUtility.jlex_assert(null != bunch.m_nfa_set);
			CUtility.jlex_assert(null != bunch.m_nfa_bit);
		}
		...
		for (i = 0; i < size; ++i) {
			...
			if (CUtility.DEBUG) {
				CUtility.jlex_assert(bunch.m_nfa_bit.get(state.m_label));
			}
		...
		while (false == nfa_stack.empty()) {
			...
			if (null != state.m_accept && state.m_label < bunch.m_accept_index) {
				...
				if (CUtility.DEBUG) {
					CUtility.jlex_assert(null != bunch.m_accept);
					CUtility.jlex_assert(
			...
			if (CNfa.EPSILON == state.m_edge) {
				if (null != state.m_next) {
					if (false == bunch.m_nfa_set.contains(state.m_next)) {
						if (CUtility.DEBUG) {
							CUtility.jlex_assert(false == bunch.m_nfa_bit.get(state.m_next.m_label));
						}
				...
				if (null != state.m_next2) {
					if (false == bunch.m_nfa_set.contains(state.m_next2)) {
						if (CUtility.DEBUG) {
							CUtility.jlex_assert(false == bunch.m_nfa_bit.get(state.m_next2.m_label));
						}
	...
	private int add_to_dstates(CBunch bunch) {
		...
		if (CUtility.DEBUG) {
			CUtility.jlex_assert(null != bunch.m_nfa_set);
			CUtility.jlex_assert(null != bunch.m_nfa_bit);
			CUtility.jlex_assert(null != bunch.m_accept || CSpec.NONE == bunch.m_anchor);
		}
...
class CInput {
	...
	boolean getLine() throws java.io.IOException {
		...
		while (true) {
			...
			m_line_read = 0;
			while ((byte) '\n' != m_buffer[m_buffer_index] && (byte) '\r' != m_buffer[m_buffer_index]) {
...
class CUtility {
	...
	/********************************************************
	  Function: jlex_assert
	  Description: Debugging routine.
	  *******************************************************/
	static void jlex_assert(boolean expr) {
		if (DEBUG && false == expr) {
...
class CLexGen {
	...
	private ObjectOutputStream o_outstream; /* Lexical analyzer data file. */
	...
	CLexGen(String filename) throws java.io.FileNotFoundException, java.io.IOException {
		...
		/* Open output streams. */
		m_outstream = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(filename + ".java")));
		if (null == m_outstream) {
		...
		try {
			o_outstream = new ObjectOutputStream(new BufferedOutputStream(new FileOutputStream(filename + ".dat")));
		}
		catch (Exception e) {
			System.out.println("Error: Unable to open output file " + filename + ".dat.");
			return;
		}
	...
	void generate() throws java.io.IOException, java.io.FileNotFoundException {
		...
		if (CUtility.DEBUG) {
			CUtility.jlex_assert(null != this);
			CUtility.jlex_assert(null != m_outstream);
			CUtility.jlex_assert(null != o_outstream);
			CUtility.jlex_assert(null != m_input);
			CUtility.jlex_assert(null != m_tokens);
			CUtility.jlex_assert(null != m_spec);
			CUtility.jlex_assert(m_init_flag);
		}
		...
		m_emit.emit(m_spec, m_outstream, o_outstream);
	...
	private void userCode() throws java.io.IOException {
		...
		while (true) {
			...
			m_outstream.writeBytes(new String(m_input.m_line, 0, m_input.m_line_read));
		}
	}
		...
		while (false == m_input.getLine()) {
			...
			if ('%' == m_input.m_line[0]) {
				...
				switch (m_input.m_line[1]) {
					case '{' :
						if (0
							== CUtility.charncmp(m_input.m_line, 0, m_class_code_dir, 0, m_class_code_dir.length - 1)) {
							m_spec.m_class_code =
								packCode(
									m_class_code_dir,
					...
					case 'i' :
						if (0 == CUtility.charncmp(m_input.m_line, 0, m_integer_dir, 0, m_integer_dir.length - 1)) {
							...
							0 == CUtility.charncmp(m_input.m_line, 0, m_intwrap_dir, 0, m_intwrap_dir.length - 1)) {
							/* Set line counting to ON. */
							m_input.m_line_index = m_integer_dir.length;
							...
							0
								== CUtility.charncmp(m_input.m_line, 0, m_init_code_dir, 0, m_init_code_dir.length - 1)) {
							m_spec.m_init_code =
								packCode(
									m_init_code_dir,
									m_init_code_end_dir,
	...
	private void userRules() throws java.io.IOException {
		...
		if (CUtility.DEBUG) {
			CUtility.jlex_assert(END_OF_INPUT == m_spec.m_current_token);
		}
	...
	void print_nfa() {
		...
		while (states.hasMoreElements()) {
			...
			if (CUtility.DEBUG) {
				CUtility.jlex_assert(null != state);
				CUtility.jlex_assert(null != index);
			}
	...
	private boolean expandMacro() {
		...
		/* Debug checks. */
		if (CUtility.DEBUG) {
			CUtility.jlex_assert(0 < count_name);
		}
		...
		for (rep_elem = 0; rep_elem < start_macro; ++rep_elem) {
			...
			if (CUtility.DEBUG) {
				CUtility.jlex_assert(rep_elem < replace.length);
			}
		}
	...
	private void saveMacro() {
		...
		in_quote = false;
		saw_escape = false;
		while (false == CUtility.isspace(m_input.m_line[elem]) || true == in_quote || true == saw_escape) {
		...
		/* Debug checks. */
		if (CUtility.DEBUG) {
			CUtility.jlex_assert(0 < count_def);
			CUtility.jlex_assert(0 < count_name);
			CUtility.jlex_assert(null != m_spec.m_macros);
		}
	...
	private void saveStates() {
		...
		/* Debug checks. */
		if (CUtility.DEBUG) {
			CUtility.jlex_assert('%' == m_input.m_line[0]);
			CUtility.jlex_assert('s' == m_input.m_line[1]);
			CUtility.jlex_assert(m_input.m_line_index <= m_input.m_line_read);
			CUtility.jlex_assert(0 <= m_input.m_line_index);
			CUtility.jlex_assert(0 <= m_input.m_line_read);
		}
	...
	CAccept packAccept() throws java.io.IOException {
		...
		if (CUtility.DEBUG) {
			CUtility.jlex_assert(null != accept);
		}
	...
	int advance() throws java.io.IOException {
		...
		/* End of previous regular expression?
		   Refill line buffer? */
		if (EOS == m_spec.m_current_token /* ADDED */
			|| m_input.m_line_index >= m_input.m_line_read)
			/* ADDED */ {
	...
	private void details() {
		...
		while (names.hasMoreElements()) {
			...
			if (CUtility.DEBUG) {
				CUtility.jlex_assert(null != name);
				CUtility.jlex_assert(null != def);
			}
		...
		if (false == m_spec.m_count_chars) {
			...
			if (CUtility.DEBUG) {
				CUtility.jlex_assert(m_spec.m_count_lines);
			}
